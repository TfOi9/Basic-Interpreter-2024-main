 ...
 * -------------------------------------
 * The functions toUpperCase and toLowerCase return a new std::string whose
 * characters appear in the desired case. These implementations rely on
 * the fact that the characters in the std::string are copied when the
 ...
 * each std::string.  Converting each std::string to uppercase and then comparing
...
int findSubString(const std::string& str,const std::string& sub){
    ...
    int len=str.size(),lenb=sub.size();
    for(int i=0;i<len-lenb+1;i++){
        if(str.substr(i,lenb)==sub){
            return i;
        }
    }
    return -1;
...
std::string cutString(const std::string& str,int l,int r){
